...
local i2c, string, issetBit, setBit, clearBit = i2c, string, bit.isset, bit.set, bit.clear
...
function mcp23017:writeIODIR(bReg, newByte)
  writeByte(self.address, self.i2cId, bReg and 0x1 --[[IODIRB register]] or 0x0 --[[IODIRA register]], newByte)
end

function mcp23017:writeGPIO(bReg, newByte)
  writeByte(self.address, self.i2cId, bReg and 0x13 --[[GPIOB register]] or 0x12 --[[GPIOA register]], newByte)
end

function mcp23017:readGPIO(bReg)
  return readByte(self.address, self.i2cId, bReg and 0x13 --[[GPIOB register]] or 0x12 --[[GPIOA register]])
end

-- read pin input
function mcp23017:getPinState(bReg, pin)
  return issetBit(
    readByte(self.address, self.i2cId, bReg and 0x13 --[[GPIOB register]] or 0x12 --[[GPIOA register]]),
...
function mcp23017:setPin(bReg, pin, state)
  ...
  writeByte(a, i, inReq, state and setBit(response, inPin) or clearBit(response, inPin))
  return true
end
...
function mcp23017:setMode(bReg, pin, mode)
  ...
  local inPin = checkPinIsInRange(pin)
  local response = readByte(a, i, inReq)
  writeByte(a, i, inReq, mode and setBit(response, inPin) or clearBit(response, inPin))
  return true
end
...
function mcp23017:setInternalPullUp(bReg, iByte)
  writeByte(self.address, self.i2cId, bReg and 0x7 --[[DEFVALB register]] or 0x6 --[[DEFVALA register]], iByte)
end
